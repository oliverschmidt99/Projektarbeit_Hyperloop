...
Im Automatikmodus wird der Ablauf über den Startknopf aktiviert. Das Fahrzeug beschleunigt auf die voreingestellte Geschwindigkeit und bremst automatisch, sobald die Distanz zum Enddeckel der Röhre kleiner wird. Es hält schließlich am Ende der Strecke an.\\
Über das Control Panel (siehe Abbildung \ref{Automat:img:Controllpanel}) können alle Eingaben via Simulink gesteuert werden. Mit dem Wahlschalter kann zwischen Automatik- und manuellem Betrieb gewechselt werden. Ist der manuelle Betrieb ausgewählt, kann mit dem Drehschalter (Manuell) zwischen Vorwärts, Rückwärts und Stopp geschaltet werden (siehe Tabelle \ref{Automat:tab:Wahlschalter}).\\

\begin{table}[!ht]
	\centering
	\caption{Automat – Wahlschalter für den manuellen Betrieb}
	\label{Automat:tab:Wahlschalter}
	\begin{tabular}{ll}
		\hline
		Zustand                    & Wert \\ \hline
		\multicolumn{1}{l|}{Rück+} & 1    \\
		\multicolumn{1}{l|}{Rück}  & 2    \\
		\multicolumn{1}{l|}{STOP}  & 3    \\
		\multicolumn{1}{l|}{Vor}   & 4    \\
		\multicolumn{1}{l|}{Vor+}  & 5    \\ \hline
	\end{tabular}
\end{table}
\pagebreak[3]
Im Automatikbetrieb kann die Geschwindigkeit über das Drehrad \frqq Geschwindigkeit Automatik\flqq\ voreingestellt oder während der Fahrt angepasst werden. Das Fahrzeug beginnt zu fahren, sobald der Druckknopf \frqq Start\flqq\ betätigt wird, und die Automatik kann über den Druckknopf \frqq Stop\flqq\ gestoppt werden.\\ Im Notfall kann sowohl im Automatik- als auch im manuellen Betrieb der Druckknopf \frqq Emergency stop\flqq\ betätigt werden. Das Fahrzeug führt eine Vollbremsung mit der Motor- und der mechanischen Bremse durch. Der Notfallmodus kann durch Drücken des \frqq Reset\flqq\ -Knopfs beendet werden.\\ \ \\
...
\begin{figure}[!ht]
	\begin{center}
		...
		\label{Automat:img:Controllpanel}
	\end{center}
\end{figure}
\pagebreak[1]

Die beiden Betriebsmodi werden im Folgenden näher erläutert.\\


Der Automat des manuellen Betriebsmodus, wie in Abbildung \ref{Automat:img:man_übersicht} dargestellt, ist in zwei Zweige unterteilt vorwärts und rückwärts. In Abbildung \ref{Automat:img:man_vorwärts} wird der Vorwärtszweig gezeigt, der Rückwärtszweig ist dazu äquivalent.
...
\begin{figure}[!ht]
	\begin{center}
		\includegraphics[width=\textwidth]{img/5_simulation/Automat_man_uebersicht.png}
		\caption{Automat – Übersicht der manuellen Steuerung}
		\label{Automat:img:man_übersicht}
	\end{center}
\end{figure}
\pagebreak[1]
...
\begin{figure}[!ht]
	\begin{center}
		\includegraphics[width=\textwidth]{img/5_simulation/Automat_man_vorwaerts_1.png}
		\includegraphics[width=\textwidth]{img/5_simulation/Automat_man_vorwaerts.png}
		\caption{Automat – Manuelle Steuerung Vorwärtszweig}
		\label{Automat:img:man_vorwärts}
	\end{center}
\end{figure}
\pagebreak[4]
%\begin{center}
...
\begin{figure}[!ht]
	\begin{center}
		\includegraphics[width=.75\textwidth]{img/5_simulation/Automat_funktion_1.png}
